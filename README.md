...
| [Mintegral](https://www.playturbo.com/review/doc) | Supported ✅ |
...
Platform integration is selected at runtime by the APIs injected into the ad webview: Facebook `FbPlayableAd`, MRAID `mraid.open`, Mintegral `window.install`, or Google `ExitApi`, in that order. MRAID takes priority over the generic `window.install` hook so AppLovin, Unity, and other MRAID hosts always open the configured store URL directly instead of a preview container's application page. Injected network APIs take priority over the bundled Google API so previews and builds used on other networks do not route their CTA through Google. MRAID may be exposed either as `window.mraid` or as a global lexical binding by a validator. MRAID startup integration additionally uses `getState` and `addEventListener` when available, while CTA routing only requires `mraid.open`. The plugin does not inject a fake `mraid` object or use the user agent to select the ad network; the user agent is only used to choose the iOS or Android store URL. Luna-specific events such as `luna:build` are only relevant when a playable is exported through Unity Playworks/Luna.
